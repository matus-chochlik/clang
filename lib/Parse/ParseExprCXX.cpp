...
static MetaobjectOp MetaobjectOpFromTokKind(tok::TokenKind kind) {
  switch (kind) {
  default: llvm_unreachable("Not a known metaobject operation");
#define METAOBJECT_OP_1(Spelling, Result, Name, Key) \
case tok::kw___metaobject_ ## Spelling: return MOO_ ## Name;
...
///
ExprResult Parser::ParseMetaobjectOperation() {

  Token OpTok = Tok;
  tok::TokenKind Kind = OpTok.getKind();
  ...
  unsigned Arity = MetaobjectOpArity(Kind);
  MetaobjectOp Operation = MetaobjectOpFromTokKind(Kind);
  MetaobjectOpResult OpResult = MetaobjectOpResultFromTokKind(Kind);

  assert((Arity > 0) && "Metaobject operation arity must be greater than zero");
  assert((Arity < 3) && "Metaobject operation arity must be less than three");

  ExprResult ArgExpr[2/*MetaobjectOpExpr::MaxArity*/];
  ...
  if(Arity > 1) {
    ...
    ArgExpr[1] = ParseCastExpression(false, false, NotTypeCast);
    if (ArgExpr[1].isInvalid())
      return ExprError();
  }

  if (Tok.is(tok::r_paren)) {
    Parens.consumeClose();
    return Actions.ActOnMetaobjectOpExpr(Operation, OpResult, Arity, ArgExpr,
                                         OpLoc, Parens.getRange());
  }
  return ExprError();
}
//
...
ExprResult Parser::ParseReflexprExpression(bool idOnly) {
  ...
  if (Tok.is(tok::r_paren)) {
    ...
    /* (named-declaration) */ {
      ...
      CXXScopeSpec SS;
      ParseOptionalCXXScopeSpecifier(SS, ParsedType(), /*EnteringCtx=*/false);
